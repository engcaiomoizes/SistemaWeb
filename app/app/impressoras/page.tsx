...
import FolhaRamal from "@/components/pdf/folhaRamal";
...
export default function Impressoras() {
    ...
    // Lógica de filtragem
    const dadosFiltrados = impressoras.filter(item => {
        const termo = normalizar(searchTerm.toLowerCase());
        const marca = item.marca ? normalizar(item.marca.toLowerCase()) : '';
        const modelo = item.modelo ? normalizar(item.modelo.toLowerCase()) : '';
        const local_nome = item.local_fk.nome ? normalizar(item.local_fk.nome.toLowerCase()) : '';
        const local_apelido = item.local_fk.nome ? normalizar(item.local_fk.apelido!.toLowerCase()) : '';

        return marca.includes(termo) || modelo.includes(termo) || local_nome.includes(termo) || local_apelido.includes(termo);
    });
    ...
    if (order === 'asc' || order === 'desc') {
        dadosOrdenados = [...dadosFiltrados].sort((a, b) => {
            ...
            if (order === 'asc') {
                return field === 'marca' ? marcaA.localeCompare(marcaB) :
                    modeloA.localeCompare(modeloB);
            } else {
                return field === 'marca' ? marcaB.localeCompare(marcaA) :
                    modeloB.localeCompare(modeloA);
            }
        });
    }
    ...
    const handleDelete = async (id: string) => {
        if (deletar) {
            try {
                ...
                if (response.ok) {
                    setMessage("Impressora deletada com sucesso!");
                    setAlertType('success');
                    handleReload();
                    setItem("");
                    setDeletar(false);
    ...
    const gerarListagem = async () => {
        try {
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_PYTHON_API_URL}/relatorio-impressoras`);

            if (!response.ok) {
                throw new Error("Erro ao gerar Relatório de Impressoras.");
            }
        ...
        <div className="flex justify-center p-10 ms-12">
            <div className="xl:container">
                <div className={`${message ? 'grid grid-cols-2' : 'flex justify-end'} items-center mb-2`}>
                    ...
                    <Link href={ `/impressoras/cadastrar` } className="justify-self-end text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-semibold rounded-lg text-xs uppercase px-5 py-2.5 focus:outline-none dark:focus:ring-blue-800 transition ease-in-out duration-100">Cadastrar</Link>
                </div>
                    ...
                    <div className="flex space-x-2">
                        {
                            ...
                            :
                            <button onClick={gerarListagem} disabled={gerando} className="cursor-pointer border px-1.5 rounded-lg border-blue-700 bg-blue-700 hover:bg-blue-800 hover:border-blue-800 transition ease-in-out duration-100">
                            ...
                            <tr>
                                <th scope="col" className="px-6 py-3">
                                    <div className="flex items-center">
                                        Marca
                                        <button className="cursor-pointer" onClick={() => handleOrder('marca')}>
                                            {
                                                field != 'marca' &&
                                ...
                                <th scope="col" className="px-6 py-3">
                                    <div className="flex items-center">
                                        Modelo
                                        <button className="cursor-pointer" onClick={() => handleOrder('modelo')}>
                                            {
                                                field != 'modelo' &&
                                                <svg className="w-3 h-3 ms-1.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
                                                    <path d="M8.574 11.024h6.852a2.075 2.075 0 0 0 1.847-1.086 1.9 1.9 0 0 0-.11-1.986L13.736 2.9a2.122 2.122 0 0 0-3.472 0L6.837 7.952a1.9 1.9 0 0 0-.11 1.986 2.074 2.074 0 0 0 1.847 1.086Zm6.852 1.952H8.574a2.072 2.072 0 0 0-1.847 1.087 1.9 1.9 0 0 0 .11 1.985l3.426 5.05a2.123 2.123 0 0 0 3.472 0l3.427-5.05a1.9 1.9 0 0 0 .11-1.985 2.074 2.074 0 0 0-1.846-1.087Z"/>
                                            ...
                                            {
                                                order == 'asc' && field == 'modelo' &&
                                                <svg className="w-3 h-3 ms-1.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                                                    <path fill-rule="evenodd" d="M18.425 10.271C19.499 8.967 18.57 7 16.88 7H7.12c-1.69 0-2.618 1.967-1.544 3.271l4.881 5.927a2 2 0 0 0 3.088 0l4.88-5.927Z" clip-rule="evenodd"/>
                                                </svg>
                                            }
                                            {
                                                order == 'desc' && field == 'modelo' &&
                                                <svg className="w-3 h-3 ms-1.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                                                    <path fill-rule="evenodd" d="M5.575 13.729C4.501 15.033 5.43 17 7.12 17h9.762c1.69 0 2.618-1.967 1.544-3.271l-4.881-5.927a2 2 0 0 0-3.088 0l-4.88 5.927Z" clip-rule="evenodd"/>
                                                </svg>
                                            }
                        ...
                        <tbody>
                            {
                                ...
                                dadosPaginados.map((item) => (
                                    <tr key={item.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                                        ...
                                        <td className="px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                                            {item.local_fk.apelido ? item.local_fk.apelido : item.local_fk.nome}
                                        </td>
                                        <td className="px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                                            <Link href={`http://${item.ip_addr}`} target="_blank" className="underline text-blue-600 hover:text-blue-800">{item.ip_addr}</Link>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <StatusDot status={item.status} />
                                        </td>
                                        <td className="px-0 py-4 text-right flex">
                                            <Link href={ `/impressoras/editar/${item.id}` } className="font-medium text-blue-600 dark:text-blue-500 hover:underline me-2">
                                ...
                                <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                                    <td className="px-6 py-4 text-center" colSpan={5}>Nenhuma Impressora cadastrada!</td>
                                </tr>
                            }
                        </tbody>
                    </table>
            ...
            <div className="relative p-4 w-full max-w-md max-h-full">
                <div className="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                    ...
                    <div className="p-4 md:p-5 text-center">
                        ...
                        <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Deseja realmente deletar este Ramal?</h3>
