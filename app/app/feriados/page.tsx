    ...
    const handleReload = useCallback(async () => {
        try {
            const response = await fetch(
                `/api/feriados?page=${currentPage}&limit=${ITEMS_PER_PAGE}${searchTerm ? `&searchTerm=${searchTerm}` : ''}${order != '' ? `&order=${order}` : ''}`
            );
            if (!response.ok) {
                throw new Error(`Erro ao buscar feriados: ${response.status} - ${response.statusText}`);
            }
            const data = await response.json();
            setFeriados(data.response);
            setCount(data.count);
    ...
    const handleDelete = async (id: number) => {
        if (deletar) {
            try {
                ...
                if (response.ok) {
                    setMessage("Feriado deletado com sucesso!");
                    setAlertType('success');
                    handleReload();
                    setItem(0);
                    setDeletar(false);
        ...
        <div className="flex justify-center p-10 ms-12">
            <div className="xl:container">
                <div className={`${message ? 'grid grid-cols-2' : 'flex justify-end'} items-center mb-2`}>
                    ...
                    <Link href={ `/feriados/cadastrar` } className="justify-self-end text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-semibold rounded-lg text-xs uppercase px-5 py-2.5 focus:outline-none dark:focus:ring-blue-800 transition ease-in-out duration-100">Cadastrar</Link>
                </div>
                        ...
                        <tbody>
                            {
                                ...
                                feriados.map((item) => (
                                    <tr key={item.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                                        ...
                                        <td className="px-0 py-4 text-right flex">
                                            <Link href={ `/feriados/editar/${item.id}` } className="font-medium text-blue-600 dark:text-blue-500 hover:underline me-2">
                                ...
                                :
                                <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                                    <td className="px-6 py-4 text-center" colSpan={3}>Nenhum Feriado cadastrado!</td>
                                </tr>
                            }
                        </tbody>
                    </table>
